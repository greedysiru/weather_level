...
export const weatherAPI = {
  ...
  fetchPreference : function(id:string){
    axios.defaults.headers.common['token'] = `${id}`;// 여기 들어가는거 괜찮나?
    return axios.get('/api/user/preferences')
  },
  createPreference : function(id:string, preferece:preferenceType){
    axios.defaults.headers.common['token'] = `${id}`;// 여기 들어가는거 괜찮나?
    return axios.post('/api/user/preferences',preferece)
  },
  updatePreference : function(id:string, preferece:preferenceType){
    axios.defaults.headers.common['token'] = `${id}`;// 여기 들어가는거 괜찮나?
