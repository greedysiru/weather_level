...
const setMonthDayTime = createAction<object>('time/SETMONTHDAYTIME');
...
const getMonthDayTime = () => (dispatch, getState) => {
  ...
  const monthDayTime: string = `${month} ${date} ${hours}`;
  // 필요한 dailyTime 인덱스 찾기
  const dailyTime: string[] = getState().weather.weatherInfo.dayInfo.dailyTime;
  let idx: number = dailyTime.indexOf(monthDayTime);
  // 현재 시간으로부터 필요한 인덱스 배열 만들기
  let timeIndex: number[] = new Array();
  for (let i = 0; i < 8; i++) {
    timeIndex[i] = idx
    idx += 3
  }
